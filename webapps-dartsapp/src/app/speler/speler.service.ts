...
export class SpelerService {
    ...
    public update(speler: Speler): Promise<Speler> {
        ...
        return this.http
            .put(url, JSON.stringify(speler), { headers: new Headers({ Authorization: `Bearer ${this.auth.token}` }) }) //{headers: this.headers}
            .toPromise()
            .then(() => speler)
    ...
    public create(n: string): Promise<Speler> {
        return this.http
            .post(this.spelersUrl, JSON.stringify({ naam: n }), { headers: new Headers({ Authorization: `Bearer ${this.auth.token}` }) })
            .toPromise()
